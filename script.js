...
function loadSettings() {
  ...
  if (savedDough) {
    ...
    if (currentDough === 'poolish') {
      const btn = document.getElementById('btnPoolish');
      ...
      const btn = document.getElementById('btnSameDay');
      btn.classList.add('active');
      btn.setAttribute('aria-pressed', 'true');
...
function setDoughType(type) {
  ...
  // Update toggle buttons UI and ARIA
  document.querySelectorAll('.dough-btn').forEach(btn => {
    btn.classList.remove('active');
    btn.setAttribute('aria-pressed', 'false');
  });
...
function calculate() {
  ...
  if (currentDough === 'poolish') {
    ...
    // SAME DAY MATH & TIP
    flourTip = `<div class="flour-tip">💡 <strong>Flour tip:</strong> For this 8-10 hour room-temperature proof, a medium-strength "00" flour (like <strong>Caputo Pizzeria / Blue, W260-270</strong>) works perfectly.</div>`;
